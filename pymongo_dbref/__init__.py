...
__version__ = "0.0.1"
__author__ = "windpro"
__author_email__ = "windprog@gmai.com"
__description__ = "auto mongodb dbref"
__url__ = "https://github.com/windprog/pymongo-dbref"
__title__ = "pymongo_dbref"
...
class Cursor(object):
    ...
    def __init__(self, collection, spec=None, fields=None, skip=0, limit=0,
        ...
        self._dereference = self.DEFAULT_DEREFERENCE
    ...
    def __dereference_stack(self, items):
        stack = LifoQueue()
        stack.put(items)
        ref_list = []
        ...
        while stack.qsize():
            ...
            if isinstance(item, list):
                for index, im in enumerate(item):
                    if isinstance(im, DBRef) and (not im.database or im.database == self.__collection.database.name):
                        ref_list.append((
                            item, index,
                            im.collection, im.id
                        ))
                ...
                for k, v in item.iteritems():
                    if isinstance(v, DBRef) and (not v.database or v.database == self.__collection.database.name):
                        ref_list.append((
                            item, k,
                            v.collection, v.id
                        ))
        ...
        if ref_list:
            ...
            for _, _, coll_name, _id in ref_list:
                if coll_name not in ref_query:
                    ref_query[coll_name] = set()
                ref_query[coll_name].add(_id)

            ref_result = {}

            for coll_name, id_list in ref_query.iteritems():
                _ims = list(self.__collection.database[coll_name].find({"_id": {
                    "$in": list(id_list)
                }}))
                for _im in _ims:
                    ref_result[(coll_name, _im["_id"])] = _im

            for owner, key, coll_name, _id in ref_list:
                if (coll_name, _id) in ref_result:
                    owner[key] = ref_result[(coll_name, _id)]
    ...
    def __send_message(self, message):
        ...
        try:
            ...
            if self._dereference:
                # dereference
                self.__dereference_stack(response)
...
class WrapperMongoClient(_MongoClient):
    def __getattr__(self, name):
        return WrapperDatabase(self, name)

    def __getitem__(self, name):
        return self.__getattr__(name)


class WrapperDatabase(_Database):
    def __getattr__(self, name):
        return WrapperCollection(self, name)

    def __getitem__(self, name):
        return self.__getattr__(name)


class WrapperCollection(_Collection):
    def find_one(self, spec_or_id=None, *args, **kwargs):
        # print "find_one"
        return super(WrapperCollection, self).find_one(spec_or_id, *args, **kwargs)
...
def patch_cursor():
    from pymongo import cursor, collection
    Cursor.DEFAULT_DEREFERENCE = True
    setattr(cursor, "Cursor", Cursor)
    setattr(collection, "Cursor", Cursor)

def unpatch_cursor():
    from pymongo import cursor, collection
    Cursor.DEFAULT_DEREFERENCE = False
    setattr(cursor, "Cursor", _Cursor)
    setattr(collection, "Cursor", _Cursor)